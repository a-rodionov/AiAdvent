...
`Session.acompletion(prompt, is_stream_prefered)` is an async generator. It SHALL: (1) call `_usage_stats.begin_invocation()` to reset per-request statistics, (2) `await self._message_context_strategy.add_user_query(prompt)` to add the user prompt to the context strategy (a pure record append — no trimming, no LLM call inside), (3) retrieve the current LLM-facing context from the strategy via `await self._message_context_strategy.get_context()` (which may, for `SummaryStrategy`, lazily perform a summarisation LLM round-trip via `LlmStatsDecorator` whose token usage is accumulated into the same `SessionUsageStats`), (4) stream the LLM response via `_llm_stats.acompletion` (the `LlmStatsDecorator`) — yielding `SessionTextChunkEvent` for each `TextChunkEvent`, (5) on `CompletionDoneEvent`, capture the stop reason, (6) `await self._message_context_strategy.add_model_response(...)` to add the assembled assistant response to the strategy, (7) yield a final `SessionCompletionDoneEvent` with the stop reason, elapsed time, and `_usage_stats.current_invocation_data`.
...
#### Scenario: acompletion awaits get_context to build the LLM request

- **WHEN** `acompletion` is called
- **THEN** the message list passed to `_llm_stats.acompletion` is the value returned by `await self._message_context_strategy.get_context()`, not `get_history()`

#### Scenario: acompletion awaits add_user_query before get_context

- **WHEN** `acompletion` is called with prompt `"q"`
- **THEN** `await self._message_context_strategy.add_user_query("q")` is executed before `await self._message_context_strategy.get_context()`, so the user prompt is part of the context the strategy projects
...
`Session.set_message_context_strategy(strategy)` SHALL replace the current strategy with a newly-built instance of the provided strategy type, carrying over all existing records from the current strategy. Existing records SHALL be fetched via `self._message_context_strategy.get_history()` — the raw-record accessor that replaces the removed `get_records()`. The new strategy SHALL be reconstructed via `MessageContextStrategyFactory.build()`, which is a synchronous call (no `await`). No strategy-application step runs at transplant time; the new strategy will lazily project its view (and, for `SummaryStrategy`, lazily summarise) on the next `await get_context()` call.
...
- **THEN** the new strategy contains every record the old strategy held; `get_history()` on the new strategy returns the same list (in order) as `get_history()` on the old strategy returned just before the switch
...
#### Scenario: factory.build is invoked synchronously inside set_message_context_strategy

- **WHEN** a developer inspects `Session.set_message_context_strategy`
- **THEN** the call to `MessageContextStrategyFactory.build(...)` is not preceded by `await`
