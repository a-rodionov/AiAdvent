...
- `update_context()` — abstract async method for performing any side-effectful update (e.g. LLM summarisation) before context is read. Strategies with no side-effectful update SHALL implement as a no-op (`pass`). Called separately from `get_context()` so that reads remain free of side effects.
- `get_context()` — abstract async method returning the ordered message dicts to send to the LLM on the next turn. Pure read — MUST NOT perform side effects.

Concrete subclasses SHALL implement `strategy_type` (str property), `get_metadata()` (returns a JSON-serialisable dict of strategy state), `update_context()` (async, for side-effectful updates), and `get_context()` (async, pure read returning ordered message dicts without the system prompt).
...
#### Scenario: get_context is awaitable and abstract

- **WHEN** a developer inspects `MessageContextStrategy.get_context`
- **THEN** the method is declared `async` and abstract; subclasses must implement it

#### Scenario: update_context is awaitable and abstract

- **WHEN** a developer inspects `MessageContextStrategy.update_context`
- **THEN** the method is declared `async` and abstract; subclasses must implement it
...
`DummyStrategy` SHALL retain all records without any pruning. Its `strategy_type` SHALL be the string `"dummy"`. Its `get_metadata()` SHALL return an empty dict. `DummyStrategy` SHALL provide an async `update_context()` that is a no-op, and an async `get_context()` returning `[record.message for record in self._records]`. `DummyStrategy` SHALL live in its own module `server/application/domain/model/context_strategy/dummy_strategy.py`.

#### Scenario: get_context returns all records in insertion order

- **WHEN** multiple messages have been added
- **THEN** `await get_context()` includes all of them in insertion order
...
- **THEN** the returned list contains exactly the message dicts of the last three records in insertion order
...
`get_context()` SHALL be `async` and SHALL be a pure read — it MUST NOT trigger any LLM calls or modify `_summary`. It SHALL return: when `_summary.anchor_id is None`, the message dicts of every record in `self._records`; otherwise `[{"role": "user", "content": self._summary.text}]` followed by the message dicts of the records appended after `_summary.anchor_id`.
...
- **THEN** a `ValueError` is raised with a message containing "LlmPort object is None"

#### Scenario: appending messages alone does not trigger summarisation

- **WHEN** records are appended via `add_user_query` / `add_model_response` and `update_context()` is never called
...
#### Scenario: anchor None and fewer than window_size records do not trigger on update_context

- **WHEN** `_summary.anchor_id is None`, `len(self._records)` is less than `window_size`, and `await update_context()` is called
- **THEN** `self._llm.acompletion` is not called and `_summary` remains unchanged

#### Scenario: anchor None and window_size records trigger first summarisation on update_context

- **WHEN** `_summary.anchor_id is None`, `len(self._records)` is greater than or equal to `window_size`, and `await update_context()` is called
...
- **WHEN** `_summary.anchor_id` is a UUID `U`, fewer than `window_size` records have been appended after `U`, and `await update_context()` is called
- **THEN** `self._llm.acompletion` is not called and `_summary` remains unchanged

#### Scenario: window_size records after anchor retrigger summarisation on update_context

- **WHEN** `_summary.anchor_id` is a UUID `U`, `window_size` or more records have been appended after `U`, and `await update_context()` is called
- **THEN** `self._llm.acompletion` is called with `[{"role":"user","content": previous_summary_text}]` followed by the records after `U`; `_summary.text` is replaced with the LLM response, and `_summary.anchor_id` is updated to the id of the most recent record

#### Scenario: records are preserved across summarisation

- **WHEN** `await update_context()` triggers summarisation
...
- **WHEN** `_summary.anchor_id is None` (no summarisation has occurred)
- **THEN** `await get_context()` returns the message dicts of every record in `self._records`

#### Scenario: get_context with anchor prepends summary and slices records after anchor

- **WHEN** `_summary.anchor_id` is a UUID `U`, records after `U` are `R1, R2`
- **THEN** `await get_context()` returns `[{"role":"user","content": self._summary.text}, R1.message, R2.message]`

#### Scenario: get_metadata carries anchor id as string

- **WHEN** `get_metadata()` is called after `await update_context()` produced anchor UUID `U`
...
- **WHEN** `await update_context()` triggers summarisation and `self._llm` is a `LlmStatsDecorator`
...
#### Scenario: no \_emit_token_usage call in update\_context

- **WHEN** a developer inspects `SummaryStrategy.update_context`
